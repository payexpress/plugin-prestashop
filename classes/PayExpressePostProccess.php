...
class PayExpressePostProccess
{
    ...
    public function send()
    {
        ...
        $params = [
            'item_name' => PayExpressePostProccess::arrayGet($this->query, 'item_name'),
            'item_price' => PayExpressePostProccess::arrayGet($this->query, 'item_price'),
            'command_name' => PayExpressePostProccess::arrayGet($this->query, 'command_name'),
            'ref_command' => $this->refCommand,
            'env' => ($this->testMode) ? 'test' : 'prod',
            'currency' => $this->currency,
            'no_calculate_fee' => $this->getNoCalculateFee(),
            'ipn_url' => PayExpressePostProccess::arrayGet($this->notificationUrl, 'ipn_url'),
            'success_url' => $this->isMobile ? $this::MOBILE_SUCCESS_URL : PayExpressePostProccess::arrayGet($this->notificationUrl, 'success_url'),
            'cancel_url' => $this->isMobile ? $this::MOBILE_CANCEL_URL : PayExpressePostProccess::arrayGet($this->notificationUrl, 'cancel_url'),
            'custom_field' => json_encode($this->customeField)
        ];

        $rawResponse = PayExpressePostProccess::post($this::URL . $this::PAYMENT_REQUEST_PATH, $params, [
            "API_KEY: {$this->apiKey}",
     ...
     * @return mixed
     */
    public function getNoCalculateFee()
    {
        return $this->noCalculateFee;
    }

    /**
     * @param mixed $noCalculateFee
     */
