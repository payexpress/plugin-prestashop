 ...
 * Time: 13:09
 */
include_once _PS_MODULE_DIR_.'payexpresse/classes/PayExpressePostProccess.php';
class PayexpressePaiementModuleFrontController extends ModuleFrontController

{
    public function postProcess()
    {
        if($this->context->cart->id != null)
        {
            ...
            $this->module->validateOrder((int)$cart->id, "3", $total, $this->module->displayName, null, array(), (int)$currency->id, false, $customer->secure_key);
        }
        ...
        $response = $paiment->setQuery([
            ...
            ->setNotificationUrl([
                'ipn_url' => Context::getContext()->link->getModuleLink('payexpresse', 'ipnpaiementconfirmation'),
                'success_url' => Context::getContext()->link->getModuleLink('payexpresse', 'paiementvalidation',array('success'=>1)) ,
                'cancel_url' =>  Context::getContext()->link->getModuleLink('payexpresse', 'paiementvalidation',array('cancel'=>1))
            ])->send();
